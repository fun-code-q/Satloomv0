...
export interface UserPresence {
  ...
  lastSeen: number
  currentActivity?: "chat" | "call" | "video-call" | "theater" | "game"
  isTyping?: boolean
...
export class UserPresenceSystem {
  ...
  async updateActivity(roomId: string, userId: string, activity?: UserPresence["currentActivity"]) {
    try {
      if (!database || this.currentUserId !== userId) return

      const activityRef = ref(database, `rooms/${roomId}/presence/${userId}`)
  ...
  async setTyping(roomId: string, userId: string, isTyping: boolean) {
    try {
      ...
      const typingRef = ref(database, `rooms/${roomId}/presence/${userId}`)
      const updateData = this.cleanPresenceData({
        isTyping,
        lastSeen: Date.now(),
      })
  ...
  async setRecordingVoice(roomId: string, userId: string, isRecording: boolean) {
    try {
      ...
      const recordingRef = ref(database, `rooms/${roomId}/presence/${userId}`)
      const updateData = this.cleanPresenceData({
  ...
  async setSendingFile(roomId: string, userId: string, isSending: boolean) {
    try {
      ...
      const sendingRef = ref(database, `rooms/${roomId}/presence/${userId}`)
      const updateData = this.cleanPresenceData({
  ...
  async setUserStatus(roomId: string, userId: string, status: UserPresence["status"]) {
    try {
      ...
      const statusRef = ref(database, `rooms/${roomId}/presence/${userId}/status`)
  ...
  private startActivityUpdater(roomId: string, userId: string) {
    ...
    this.activityInterval = setInterval(async () => {
      if (this.currentUserId === userId && database) {
        try {
          const lastSeenRef = ref(database, `rooms/${roomId}/presence/${userId}/lastSeen`)
